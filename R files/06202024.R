...
current_date <- as.Date("2024-06-20")

######################################################### DSX List ######################################################

dsx_lag1 <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2023/DSX Forecast Backup - 2023.11.01.xlsx")

dsx_1 <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2023/DSX Forecast Backup - 2023.12.04.xlsx")
dsx_2 <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.01.02.xlsx")
dsx_3 <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.02.01.xlsx")
dsx_4 <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.03.01.xlsx")
dsx_5 <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.04.01.xlsx")
dsx_6 <- read_excel("S:/Global Shared Folders/Large Documents/S&OP/Demand Planning/BI Forecast Backup/2024/DSX Forecast Backup - 2024.05.01.xlsx")

######################################################### Other List ######################################################

# BoM RM to sku 
rm_to_sku <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/RM/Weekly Report run/2024/05.28.2024/Raw Material Inventory Health (IQR) NEW TEMPLATE - 05.28.2024.xlsx", 
                        sheet = "RM to SKU")

# BoM Report 
bom <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/IQR Automation/RM/Weekly Report run/2024/05.28.2024/Raw Material Inventory Health (IQR) NEW TEMPLATE - 05.28.2024.xlsx", 
                  sheet = "BoM")
...
sku_actual <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Oil Consumption/12 month rolling report/2024.05 Lag4/shipped.xlsx")

# Input sales orders (Make sure in the MSTR if months info input correct) 
# https://edgeanalytics.venturafoods.com/MicroStrategyLibrary/app/DF007F1C11E9B3099BB30080EF7513D2/7D421DDA4D4411DA73B4469771826BD9/W62--K46
sales_orders <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Oil Consumption/12 month rolling report/2024.05 Lag4/ordered.xlsx")


# Completed SKU List 
completed_sku_list <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Safety Stock Compliance/Weekly Run Files/2024/05.28.2024/Completed SKU list - Linda.xlsx")
...
# Exception Report
exception_report <- read_excel("C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 23/Safety Stock Compliance/Weekly Run Files/2024/06.18.2024/exception report.xlsx")

# Supplier Address
supplier_address <- read_excel("S:/Supply Chain Projects/Data Source (SCE)/Address Book/Address Book - 2024.06.03.xlsx",
                               sheet = "supplier")
...
final_paper <- final_paper %>% 
  dplyr::filter(!(Location %in% c(-1, 690, 691, 39, 292)))
...
ss_optimization_raw[-1:-5,] -> ss_optimization_raw
colnames(ss_optimization_raw) <- ss_optimization_raw[1, ]
ss_optimization_raw[-1, ] -> ss_optimization_raw


ss_optimization_raw %>% 
  janitor::clean_names() %>% 
  dplyr::select(item, class, item_type) %>% 
  dplyr::distinct(item, .keep_all = TRUE) %>% 
  dplyr::rename(Component = item, Class = class, "Item Type" = item_type) %>% 
  dplyr::mutate(Component = as.double(Component)) -> ss_optimization_raw_class_type



final_paper %>%
  left_join(
    ss_optimization_raw_class_type %>% group_by(Component) %>% slice(1) %>% ungroup(), 
    by = "Component") -> final_paper
...
final_paper %>% 
  dplyr::mutate(ref_component = paste0(Location, "_", Component)) %>% 
  dplyr::left_join(
...
##### Final Touch
final_paper %>% 
  dplyr::rename("Forecasted Component Qty (Lag 0)" = "Forecasted Oil Qty (Lag 0)",
                "Forecasted Component Qty (Lag 1)" = "Forecasted Oil Qty (Lag 1)") -> final_paper




write_xlsx(final_paper, "C:/Users/slee/OneDrive - Ventura Foods/Ventura Work/SCE/Project/FY 25/Raw consumption Lag 1/Monthly recurring reports/06.20.2024/raw consumption comparison.xlsx")
